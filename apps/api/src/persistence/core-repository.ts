  ...
  async persist(snapshot: CoreHydration): Promise<void> {
    ...
    try {
      ...
      for (const space of snapshot.spaces) {
        ...
        if (space.inviteHash && space.inviteExpiresAt) { await client.query("delete from invitations where space_id=$1 and accepted_by is null", [space.id]); await client.query("insert into invitations(id,space_id,created_by,token_hash,expires_at) values ($1,$2,$3,$4,$5) on conflict (id) do update set token_hash=excluded.token_hash,expires_at=excluded.expires_at,accepted_by=null,accepted_at=null", [`invite-${space.id}-${space.inviteHash.slice(0, 12)}`, space.id, space.members[0], space.inviteHash, space.inviteExpiresAt]); }
