...
import { WegoActionSchema, canEquipItem, canRewardAction, createInitialRewardGuards, createInitialWorld, markRewardedAction, normalizeWorld, reduceWorld, roomSlots } from "@wego/domain";
...
import { MemoryCoreRepository, PostgresCoreRepository, type CoreHydration, type CoreRepository } from "./persistence/core-repository";
...
function creditWorldReward(userId: string, reward: NonNullable<ReturnType<typeof reduceWorld>["reward"]>, action: { type: string; objectId?: string; at: string }): boolean {
  const guardedAction = action.type === "pet" || action.type === "memory" || action.type === "play" ? action.type : null;
  if (guardedAction) {
    const currentDate = action.at.slice(0, 10);
    const currentGuards = rewardGuards.get(userId);
    const guards = currentGuards?.date === currentDate ? currentGuards : createInitialRewardGuards(currentDate);
    const economy = { wallet: walletForUser(userId), ledger: sparkLedgers.get(userId) ?? [], entitlements: [], purchases: [], fulfilledKeys: [], rewardGuards: guards };
    if (!canRewardAction(economy, { action: guardedAction, amount: reward.amount, at: action.at, gameId: action.objectId }).allowed) return false;
    if (!creditSparks(userId, reward.amount, reward.reason, reward.idempotencyKey, { action: action.type })) return false;
    rewardGuards.set(userId, markRewardedAction(economy, { action: guardedAction, amount: reward.amount, at: action.at, gameId: action.objectId }).rewardGuards);
    return true;
  }
  return creditSparks(userId, reward.amount, reward.reason, reward.idempotencyKey, { action: action.type });
}
...
export function buildServer(options: BuildServerOptions = {}) {
  for (const map of [users, spaces, checkins, sessions, stories, planned, wallets, sparkLedgers, purchases, entitlements, rewardGuards]) map.clear();
  ...
  let store: Store;
  let core: CoreRepository;
  if (nodeEnv !== "production") { store = new MemoryStore((spaceId, userId) => spaces.get(spaceId)?.members.includes(userId) ?? false); core = new MemoryCoreRepository(); }
  else {
  ...
  app.post("/v1/auth/telegram", async (request, reply) => { const body = z.object({ initData: z.string().default("") }).parse(request.body); if (!body.initData && !demo) throw new ApiError(401, "TELEGRAM_INIT_DATA_REQUIRED", "Откройте WEGO из Telegram"); let telegramUser: { id: number; first_name: string; language_code?: string } = { id: 0, first_name: "Локальный пользователь", language_code: "en" }; if (body.initData) { try { telegramUser = validateTelegramInitData(body.initData, config.botToken); } catch { throw new ApiError(401, "INVALID_TELEGRAM_INIT_DATA", "Не удалось подтвердить Telegram"); } } const id = `tg-${telegramUser.id}`; const user = users.get(id) ?? { id, telegramId: telegramUser.id, name: telegramUser.first_name, tone: "coral" as const, regionTier: resolveRegionTier(telegramUser.language_code) }; user.regionTier = resolveRegionTier(telegramUser.language_code); users.set(id, user); const token = randomBytes(24).toString("hex"); const expiresAt = runtimeNow().getTime() + (options.sessionTtlMs ?? 30 * 86400000); sessions.set(token, { userId: id, expiresAt, serverId }); await core.persist({ users: [user], spaces: [], checkins: [], stories: [], planned: [], wallets: [], ledgers: [], worlds: [], rewardGuards: [] }); await core.saveSession(token, id, expiresAt); const secure = nodeEnv === "production" ? "; Secure" : ""; const sameSite = nodeEnv === "production" ? "None" : "Lax"; reply.header("Set-Cookie", `wego_session=${encodeURIComponent(token)}; HttpOnly; SameSite=${sameSite}; Path=/; Max-Age=${Math.floor((expiresAt - runtimeNow().getTime()) / 1000)}${secure}`).send({ user, token }); });
  app.post("/v1/auth/logout", async (request, reply) => { userFromRequest(request as never); const token = cookieValue(String(request.headers.cookie ?? ""), "wego_session") ?? String(request.headers.authorization ?? "").replace(/^Bearer /, ""); sessions.delete(token); await core.deleteSession(token); return reply.header("Set-Cookie", "wego_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0").send({ ok: true }); });
  const hydrateMaps = (hydration: CoreHydration) => { for (const user of hydration.users) users.set(user.id, user); for (const space of hydration.spaces) spaces.set(space.id, space); for (const checkin of hydration.checkins) { const space = hydration.spaces.find((item) => item.members.includes(checkin.userId)); if (space) checkins.set(`${space.id}:${checkin.date}:${checkin.userId}`, checkin as Checkin); } for (const story of hydration.stories) stories.set(story.id, story as Story); for (const item of hydration.planned) planned.set(item.spaceId, new Set(item.activityIds)); for (const wallet of hydration.wallets) wallets.set(wallet.userId, wallet); for (const userId of hydration.wallets.map((item) => item.userId)) sparkLedgers.set(userId, hydration.ledgers.filter((item) => item.userId === userId)); for (const guards of hydration.rewardGuards) rewardGuards.set(guards.userId, guards.state); for (const world of hydration.worlds) worldState.restore(world.spaceId, { world: normalizeWorld(world.world), revision: world.revision }); };
  const snapshotForUser = (userId: string): CoreHydration => { const userSpaces = Array.from(spaces.values()).filter((space) => space.members.includes(userId)); const ids = new Set(userSpaces.flatMap((space) => space.members)); return { users: Array.from(users.values()).filter((user) => ids.has(user.id)), spaces: userSpaces, checkins: Array.from(checkins.entries()).filter(([key]) => userSpaces.some((space) => key.startsWith(`${space.id}:`))).map(([, value]) => value), stories: Array.from(stories.values()).filter((story) => userSpaces.some((space) => story.sourceId.startsWith(`${space.id}:`))), planned: userSpaces.map((space) => ({ spaceId: space.id, activityIds: Array.from(planned.get(space.id) ?? []) })), wallets: Array.from(wallets.values()).filter((wallet) => ids.has(wallet.userId)), ledgers: Array.from(sparkLedgers.values()).flat().filter((entry) => ids.has(entry.userId)), worlds: userSpaces.map((space) => { const record = worldState.read(space.id); return { spaceId: space.id, world: record.world, revision: record.revision }; }), rewardGuards: Array.from(rewardGuards.entries()).filter(([guardUserId]) => ids.has(guardUserId)).map(([guardUserId, state]) => ({ userId: guardUserId, state })) }; };
  app.addHook("preHandler", async (request) => { if (!request.url.startsWith("/v1/") || request.url.startsWith("/v1/auth/telegram") || request.url.startsWith("/v1/telegram/webhook")) return; const token = cookieValue(String(request.headers.cookie ?? ""), "wego_session") ?? String(request.headers.authorization ?? "").replace(/^Bearer /, ""); if (nodeEnv === "production" && token) { const session = await core.session(token); if (session) { hydrateMaps(await core.hydrateUser(session.userId)); sessions.set(token, { ...session, serverId }); } } const invite = /^\/v1\/invitations\/([^/?]+)/.exec(request.url); if (nodeEnv === "production" && invite) hydrateMaps(await core.hydrateInvite(decodeURIComponent(invite[1]!))); const user = userFromRequest(request as never); (request as unknown as { __wegoUserId?: string }).__wegoUserId = user.id; });
  app.addHook("onResponse", async (request) => { if (nodeEnv !== "production") return; const userId = (request as unknown as { __wegoUserId?: string }).__wegoUserId; if (userId) await core.persist(snapshotForUser(userId)); });
  ...
  app.post("/v1/world/commands", async (request) => { const user = userFromRequest(request as never); const input = worldCommandSchema.parse(request.body); const key = worldKeyForUser(user.id); const action = WegoActionSchema.parse({ ...input.action, id: input.commandId, actorId: user.id, at: runtimeNow().toISOString() }); const mutation = worldState.execute(key, { commandId: input.commandId, expectedRevision: input.expectedRevision, action }); if (mutation.status === "conflict") throw new ApiError(409, "WORLD_REVISION_CONFLICT", "Комната уже изменилась на другом устройстве", { revision: mutation.record.revision, world: mutation.record.world }); const acceptedReward = mutation.status === "applied" && mutation.result?.reward && creditWorldReward(user.id, mutation.result.reward, action) ? mutation.result.reward : null; const activeSpace = activeSpaceForUser(user.id); if (activeSpace && mutation.status === "applied") eventHub.publish({ type: "world_changed", spaceId: activeSpace.id, version: mutation.record.revision }); return { world: mutation.record.world, revision: mutation.record.revision, replayed: mutation.status === "replayed", action, reward: acceptedReward, wallet: walletForUser(user.id), message: mutation.result?.message ?? "Действие сохранено." }; });
  app.post("/v1/world/actions", async (request) => { const user = userFromRequest(request as never); const parsed = WegoActionSchema.parse(request.body); const action = { ...parsed, actorId: user.id, at: runtimeNow().toISOString() }; const key = worldKeyForUser(user.id); const mutation = worldState.execute(key, { commandId: action.id, expectedRevision: worldState.read(key).revision, action }); const acceptedReward = mutation.status === "applied" && mutation.result?.reward && creditWorldReward(user.id, mutation.result.reward, action) ? mutation.result.reward : null; return { world: mutation.record.world, revision: mutation.record.revision, action, reward: acceptedReward, wallet: walletForUser(user.id), message: mutation.result?.message ?? "Действие сохранено." }; });
