...
import { getTelegramContext, haptic } from "../../lib/telegram";
...
function InviteStep({ url, copied, onCopy, onNext }: { url: string; copied: boolean; onCopy: () => void; onNext: () => void }) { return <div className="onboarding__step"><div className="onboarding__copy"><div className="w-mono-caps">Шаг 3 из 4</div><h2 className="w-serif">Пригласите<br />второго участника</h2><p>Отправьте ссылку — как только они присоединятся, ваш Wego появится.</p><WCard tone="lilac" className="invite-card"><code>{url}</code><button type="button" onClick={onCopy}>{copied ? "Скопировано" : "Копировать"}</button></WCard></div><WButton size="xl" onClick={onNext}>Продолжить →</WButton></div>; }
