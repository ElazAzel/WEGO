...
export function SettingsPage() {
  ...
  return <div className="app-page"><PageHeader eyebrow="Настройки" title="Ваш Wego" description="Здесь видно, кто уже в вашей комнате и как пригласить второго участника." /><div className="settings-list">
    <WCard tone="lilac"><div className="w-mono-caps">Аккаунт</div><div className="settings-account"><strong>{me?.name ?? "Telegram-пользователь"}</strong><span>{isApiEnabled() ? "Telegram подключён · данные синхронизируются" : "Локальный предпросмотр · данные только на этом устройстве"}</span></div><div className="settings-partner"><span className="w-mono-caps">Партнёр</span><strong>{partner?.name ?? "Ещё не подключён"}</strong>{partner ? <small>Общая комната активна: вы оба видите одного Wego и его историю.</small> : <small>Отправьте приглашение. После входа партнёра через Telegram он появится здесь.</small>}</div>{!partner && <WButton size="sm" loading={busy} onClick={() => void invitePartner()}>Пригласить партнёра</WButton>}{inviteUrl && <code className="settings-invite-url">{inviteUrl}</code>}{error && <p className="onboarding-error" role="alert">{error}</p>}<WButton size="sm" variant="secondary" onClick={() => void logout()}>Выйти из аккаунта</WButton></WCard>
    {import.meta.env.DEV && <><WCard tone="paper"><div className="w-mono-caps">Визуальный стиль</div><div className="chip-wrap settings-options"><WChip active={activeSpace.style === "a"} onClick={() => setStyle("a")}>Style A · Premium</WChip><WChip active={activeSpace.style === "b"} onClick={() => setStyle("b")}>Style B · Cozy</WChip></div></WCard>
