...
export function GuessSheet() {
  ...
  return <BottomSheet open={open} onClose={closeSheet} title="Guess" height="82%"><p className="w-serif guess-title">Как думаешь, как сегодня {partnerName}?</p><p className="muted-copy">Твой ответ скрыт от {partnerName} до Reveal.</p>{error && <p className="onboarding-error" role="alert">{error}</p>}<div className="chip-wrap guess-chips">{MOOD_OPTIONS.map((item) => <WChip key={item.id} tone={item.tone} active={guess === item.id} onClick={() => setGuess(item.id)}>{item.emoji} {item.label}</WChip>)}</div><div className="sheet-sticky-cta"><WButton size="xl" loading={busy} disabled={!guess || busy} onClick={() => void submit()}>Сохранить догадку →</WButton></div></BottomSheet>;
}
