...
Создан отдельный Vercel-проект `wego-api`. Публичный runtime-домен для Telegram: [wego-api-alpha.vercel.app](https://wego-api-alpha.vercel.app); командный alias оставлен под Vercel Authentication. В deployment уже есть Fastify entrypoint, но runtime намеренно не считается готовым, пока в Vercel не заданы приватные env-переменные из `apps/api/.env.example`.
